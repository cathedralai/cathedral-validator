...
from cathedral_thin.independent_runtime.gpu_qualification import (
    AccessRequestSigner, GpuPrelaunchConfig, GpuRound, ProductionGpuVerifier,
    atomic_json, direct_gpu_plan, provider_directory, qualify_gpu_round,
)
...
@pytest.fixture
def config(tmp_path):
    return GpuPrelaunchConfig("test", 123, ALICE.ss58_address, 2, str(tmp_path / "registry.json"),
        {"root": b"r" * 32}, 1, str(tmp_path / "registry.sqlite"), ("test-profile",))

class HardwareFreeVerifier:
    registry_digest = "sha256:" + "b" * 64
    profiles = {"test-profile": SimpleNamespace(profile_id="test-profile",
        expected_device_identity_digests={DEVICE}, allowed_models={"test-device"})}
    ...
    def verify(self, components, nonce, hotkey, binding, profile_id):
        ...
        assert components == [{"nonce": nonce.hex(), "hotkey": hotkey, "binding": binding.hex()}]
        self.nonces.append(nonce)
        return {"device_identity_digests": (DEVICE,),
                "machine_id": "changed" if self.change_completion and len(self.nonces) > 1 else "machine",
...
class HardwareFreeHttps(HttpsEvidenceTransport):
    ...
    def post_authorized(self, url, body, authorization):
        ...
        path = auth["path"]
        if path == "/v1/fleet":
            answer = {"schema": "cathedral_worker_fleet_v1", "worker_hotkey": hotkey,
                      "endpoints": [url.rsplit("/", 2)[0]]}
        elif path == "/v1/gpu-capabilities":
            answer = {"schema": "cathedral_gpu_capability_v1", "profile_id": "test-profile",
                "device_identity_digests": [DEVICE], "workload_id": "cuda_i32_vector_v1",
                "elements": 4096, "status": "registered", "verified": False}
        elif path == "/v1/gpu-evidence":
            answer = {"schema": "cathedral_gpu_evidence_v1", "evidence": evidence(body["nonce_hex"])}
        else:
            output = expected_output_digest(body)
            answer = {"schema": "cathedral_gpu_result_v1", "request_digest": request_digest(body),
...
def qualify(config, miners, verifier=None):
    return qualify_gpu_round(miners, config=config, keypair=ALICE,
        verifier=verifier or HardwareFreeVerifier(), transport_factory=HardwareFreeHttps)
...
def test_fresh_completion_signed_access_and_explicit_test_plan(config, miner):
    ...
    assert HardwareFreeHttps.calls == ["/v1/fleet", "/v1/gpu-capabilities", "/v1/gpu-evidence", "/v1/gpu-work"]
...
def test_same_gpu_under_two_hotkeys_zeros_both_before_work(config, miner):
    ...
    assert all(row["eligible"] is False and row["reason"] == "duplicate_gpu_or_channel" for row in result.rows)
...
def test_no_positive_units_for_bad_work_or_changed_completion(config, miner, failure):
    ...
    result = qualify(config, [miner], HardwareFreeVerifier(change_completion=failure == "identity"))
...
def test_duplicate_positive_rows_cannot_enter_plan(config, miner):
    ...
    with pytest.raises(IndependentLiveError, match="invalid identity"):
        direct_gpu_plan(replace(result, rows=result.rows + result.rows), config, [miner])
...
def test_config_requires_explicit_enabled_and_conversion(config, tmp_path):
    ...
    data = {"schema": "cathedral_gpu_prelaunch_v1", "enabled": True, "network": "test",
            "netuid": 123, "validator_hotkey": ALICE.ss58_address, "units_per_device": 1,
            "registry_path": config.registry_path, "trusted_keys_hex": {"root": "72" * 32},
            "minimum_registry_release": 1, "registry_state_path": config.registry_state_path,
            "profile_ids": ["test-profile"]}
    path.write_text(json.dumps(data))
    assert GpuPrelaunchConfig.load(path).units_per_device == 1
    for key, value in [("enabled", False), ("units_per_device", True), ("netuid", True)]:
...
def test_listing_aggregates_workers_and_keeps_admission_proof_on_work_failure(config, miner):
    HardwareFreeHttps.wrong_output = True
    verifier = HardwareFreeVerifier()
    result = qualify(config, [miner], verifier)
    directory = provider_directory(replace(result, rows=result.rows + result.rows), config, verifier)
...
def test_unknown_fleet_cannot_publish_empty_inventory(config):
    result = GpuRound(({"uid": 9, "profile_id": None, "reason": "fleet_unavailable"},), "now", "sha256:" + "b" * 64)
...
def g4_rows(miner, count=8):
    from cathedral_thin.independent_runtime.gpu_qualification import G4_WORKER_PROFILE_ID
    return [{"uid": miner.uid, "hotkey": miner.hotkey, "profile_id": G4_WORKER_PROFILE_ID,
        "verified": True, "eligible": True, "reason": "verified_work", "gpu_count": 1,
        "device_identity_digests": (f"sha256:{index:064x}",),
        "declared_device_identity_digests": (f"sha256:{index:064x}",),
        "provider_instance_id": f"project/zone/{index}", "worker_key_digest": f"sha256:{index:064x}", "endpoint": f"https://8.8.8.{index}:443",
        "channel_id": f"{index:064x}", "evidence_digest": "sha256:" + "e" * 64,
        "admission_digest": "sha256:" + "a" * 64, "verified_at": "2026-09-16T00:00:00Z"}
...
def test_g4_repeated_instance_and_mixed_hotkeys_do_not_form_bundle(config, miner):
    ...
    assert direct_gpu_plan(replace(result, rows=tuple(rows)), config, [miner, other])["weights"] == []


def test_g4_public_directory_reports_partial_without_host_attestation_claim(config, miner):
    from cathedral_thin.independent_runtime.gpu_qualification import G4_WORKER_PROFILE_ID, G4_BUNDLE_PROFILE_ID
    verifier = SimpleNamespace(profiles={G4_WORKER_PROFILE_ID: SimpleNamespace(profile_id=G4_WORKER_PROFILE_ID,
        expected_device_identity_digests=(), allowed_models={"NVIDIA RTX PRO 6000 Blackwell Server Edition"})})
...
def test_g4_config_embeds_explicit_operator_roots_and_cannot_select_tdx_verifier(tmp_path):
    path = tmp_path / "g4.json"
    path.write_text(json.dumps({"schema": "cathedral_gpu_g4_prelaunch_v1", "enabled": True,
        "network": "test", "netuid": 123, "validator_hotkey": ALICE.ss58_address,
        "units_per_device": 1, "trusted_operators_hex": {"operator": "72" * 32}}))
...
def test_real_g4_signatures_bind_eight_instance_work_bundle(config, miner):
    ...
    from cathedral.gpu_provider import (G4ProviderVerifier, G4_WORKER_PROFILE_ID,
        ENDORSEMENT_SCHEMA, STATEMENT_SCHEMA, MODEL, sign_endorsement)
    operator = Ed25519PrivateKey.generate()
    public = operator.public_key().public_bytes(Encoding.Raw, PublicFormat.Raw)
    config = replace(config, trusted_operators={"approved": public}, profile_ids=(G4_WORKER_PROFILE_ID,))
    ...
    uuids = {endpoint: f"GPU-00000000-0000-4000-8000-{i:012x}" for i, endpoint in enumerate(endpoints, 1)}
    ...
    class G4Https(HttpsEvidenceTransport):
        ...
        def post_authorized(self, url, body, authorization):
            ...
            if path == "/v1/fleet":
                answer = {"schema": "cathedral_worker_fleet_v1", "worker_hotkey": miner.hotkey, "endpoints": endpoints}
            elif path == "/v1/gpu-capabilities":
                answer = {"schema": "cathedral_gpu_capability_v1", "profile_id": G4_WORKER_PROFILE_ID,
                    "device_identity_digests": [gpu_identity_policy_digest(uuids[origin])],
                    "workload_id": "cuda_i32_vector_v1", "elements": 4096, "status": "registered", "verified": False}
            else:
                def evidence(nonce):
                    now = int(time.time())
                    claims = {"schema": ENDORSEMENT_SCHEMA, "profile_id": G4_WORKER_PROFILE_ID,
                        ...
                        "hotkey": miner.hotkey, "machine_type": "g4-standard-48", "provisioning_model": "SPOT",
                        "confidential_compute_type": "SEV", "cpu_attestation": "unattested",
                        "guest_control": "approved_operator", "private_customer_work": False,
                        "image_digest": "sha256:" + "a" * 64,
                        "worker_public_key_hex": private[origin].public_key().public_bytes(Encoding.Raw, PublicFormat.Raw).hex(),
                        "tls_spki_sha256": self.last_spki.hex(), "gpu_uuid": uuids[origin],
                        "issued_at": now-10, "expires_at": now+300}
                    endorsed = sign_endorsement(claims, "approved", operator)
                    statement = {"schema": STATEMENT_SCHEMA,
                        "endorsement_digest": "sha256:" + hashlib.sha256(canonical(endorsed)).hexdigest(),
                        "nonce_hex": nonce.hex(), "hotkey": miner.hotkey, "tls_spki_sha256": self.last_spki.hex(),
                        "issued_at": now, "gpu_uuid": uuids[origin], "gpu_model": MODEL,
                        "cc_mode": "ON", "ready_state": "ready", "verifier": "nv-local-gpu-verifier",
                        "verifier_version": "2.7.3", "verifier_log_b64": base64.b64encode(logs).decode(),
                        "verifier_log_digest": "sha256:" + hashlib.sha256(logs).hexdigest()}
                    return {"endorsement": endorsed, "statement": statement,
                        "signature_hex": private[origin].sign(STATEMENT_SCHEMA.encode()+b"\0"+canonical(statement)).hex()}
                if path == "/v1/gpu-evidence":
                    answer = {"schema": "cathedral_gpu_provider_evidence_v1", "evidence": evidence(bytes.fromhex(body["nonce_hex"]))}
                else:
                    output = expected_output_digest(body)
                    answer = {"schema": "cathedral_gpu_result_v1", "request_digest": request_digest(body),
                        "output_digest": output, "device_identity_digests": body["device_identity_digests"],
                        "completion_evidence": evidence(completion_nonce(body, output))}
            return 200, json.dumps(answer).encode()
    verifier = G4ProviderVerifier({"approved": public})
    result = qualify_gpu_round([miner], config=config, keypair=ALICE, verifier=verifier, transport_factory=G4Https)
    ...
    denied = qualify_gpu_round([miner], config=config, keypair=ALICE, verifier=untrusted, transport_factory=G4Https)
