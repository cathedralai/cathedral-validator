...
def test_unknown_fleet_cannot_publish_empty_inventory(config):
    result = GpuRound(({"profile_id": None, "reason": "fleet_unavailable"},), "now", "sha256:" + "b" * 64)
    document = provider_directory(result, config, HardwareFreeVerifier())
    assert document["schema"] == "cathedral.gpu.providers.unavailable.v1"
...
def g4_rows(miner, count=8):
    ...
    return [{"uid": miner.uid, "hotkey": miner.hotkey, "profile_id": G4_WORKER_PROFILE_ID,
        ...
        "provider_instance_id": f"project/zone/{index}", "endpoint": f"https://8.8.8.{index}:443",
