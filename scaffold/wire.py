...
_SIGNATURE_EXCLUDED_KEYS = frozenset(
    {"signature", "cathedral_signature", "merkle_epoch"}
)
...
SIGNED_KEYS_BY_VERSION: dict[int, frozenset[str]] = {
    5: SIGNED_KEYS_V5,
    6: SIGNED_KEYS_V6,
}
...
def sign_row(row: dict[str, Any], private_key_hex: str) -> str:
    ...
    return base64.b64encode(sk.sign(canonical_json(signed_payload(row)))).decode(
        "ascii"
    )
