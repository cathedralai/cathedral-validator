...
def _upload_headers(
    kp, blob: bytes, *, submitted_at: str | None = None
) -> dict[str, str]:
    ts = submitted_at or _now_iso()
    blob_sha = hashlib.sha256(blob).hexdigest()
    sig = base64.b64encode(
        kp.sign(
            solution_manifest.canonical_blob_upload_bytes(
...
def _headers(kp, body: dict, *, submitted_at: str | None = None) -> dict[str, str]:
    ...
    sig = base64.b64encode(
        kp.sign(solution_manifest.canonical_manifest_bytes(manifest))
    ).decode("ascii")
    return {
        "X-Cathedral-Hotkey": kp.ss58_address,
        "X-Cathedral-Signature": sig,
...
def _v1_submit_headers(
    kp, *, challenge_id: str, solution: str, submitted_at: str | None = None
) -> dict[str, str]:
    ts = submitted_at or _now_iso()
...
def _build(
    ...
    *,
    ...
    monkeypatch.setenv(
        "CATHEDRAL_V2_BLOB_UPLOAD_ENABLED", "true" if enabled else "false"
    )
    monkeypatch.setenv(
        "CATHEDRAL_V2_SHADOW_V1_ENABLED", "true" if shadow_v1 else "false"
    )
    monkeypatch.setenv("CATHEDRAL_V2_SHADOW_V1_MAX_SOLUTION_BYTES", "1000000")
    monkeypatch.setenv(
        "CATHEDRAL_V2_SUBMIT_BITSET_ENABLED", "true" if bitset_submit else "false"
    )
    monkeypatch.setenv(
...
def test_solution_manifest_v2_shadow_v1_submit_default_off(tmp_path, monkeypatch):
    app, _store = _build(
        tmp_path, monkeypatch, enabled=True, role="all", shadow_v1=False
    )
...
def test_solution_manifest_v2_shadow_v1_submit_admits_storage_only(
    ...
    receipt = client.get(
        f"/v2/shadow/v1/agents/submit/receipts/{payload['receipt_id']}"
    )
...
def test_solution_manifest_v2_shadow_v1_submit_meta_admits_without_body(
    tmp_path, monkeypatch
):
    ...
    r = client.post(
        ...
        headers={
            "X-Cathedral-Hotkey": kp.ss58_address,
            "X-Cathedral-Shadow-Source": "test-edge-meta",
        },
    )
...
def test_solution_manifest_v2_serves_prefixed_pm_challenges_and_cnf(
    tmp_path, monkeypatch
):
...
def test_solution_manifest_v2_submit_bitset_e2e_scores_shadow_weights(
    tmp_path, monkeypatch
):
    app, _store = _build(
        tmp_path,
    ...
    vector = weights.json()
    row = next(
        (w for w in vector["weights"] if w["miner_hotkey"] == kp.ss58_address), None
    )
    ...
    assert any(
        r["id"] == receipt["receipt_id"] and r["source"] == "bitset"
...
def test_solution_manifest_v2_submit_bitset_backpressure_sheds_new_work_not_replays(
    tmp_path, monkeypatch
):
...
def test_solution_manifest_v2_manifest_and_bitset_same_challenge_count_once(
    tmp_path, monkeypatch
):
    app, _store = _build(
        tmp_path,
    ...
    uploaded = client.post(
        "/v2/blobs/solutions", content=blob, headers=_upload_headers(kp, blob)
    )
    ...
    tick = client.post(
        "/v2/admin/verify/tick", headers={"Authorization": "Bearer test-admin-token"}
    )
...
def test_solution_manifest_v2_submit_bitset_rejects_bad_shape_without_row(
    tmp_path, monkeypatch
):
    app, _store = _build(
        tmp_path,
...
def test_solution_manifest_v2_submit_bitset_rejects_auth_failures_without_rows(
    tmp_path, monkeypatch
):
    app, _store = _build(
        tmp_path,
    ...
    bad_sig_headers["X-Cathedral-Signature"] = base64.b64encode(b"0" * 64).decode(
        "ascii"
    )
    bad_sig = client.post(
        "/v2/agents/submit-bitset", json=valid_body, headers=bad_sig_headers
    )
    ...
    forged = {
        **valid_body,
        "submit_token": item["submit_token"][:-1]
        + ("A" if item["submit_token"][-1] != "A" else "B"),
    }
    forged_r = client.post(
        "/v2/agents/submit-bitset", json=forged, headers=_bitset_headers(kp, forged)
    )
    ...
    expired_r = client.post(
        "/v2/agents/submit-bitset", json=expired, headers=_bitset_headers(kp, expired)
    )
...
def test_solution_manifest_v2_accepts_signed_manifest_and_receipt(
    tmp_path, monkeypatch
):
...
def test_solution_manifest_v2_rejects_unknown_encoding(tmp_path, monkeypatch):
    ...
    r = client.post(
        "/v2/agents/submit-manifest", json=body, headers=_headers(kp, _body())
    )
...
def test_solution_manifest_v2_blob_to_verified_receipt_weights_and_audit(
    tmp_path, monkeypatch
):
    ...
    admitted = client.post(
        "/v2/agents/submit-manifest", json=body, headers=_headers(kp, body)
    )
    ...
    assert vector["weights"] == [
        {"miner_hotkey": kp.ss58_address, "weight": 1.0, "raw_score": 1.0}
    ]
