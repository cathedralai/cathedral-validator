...
def test_verified_bitset_event_bridges_to_per_miner_solves(tmp_path, monkeypatch):
    ...
    page = client.get(
        "/v2/synthetic-boolean/per-miner/challenges?limit=1", headers=_read_headers(kp)
    )
    ...
    def _insert(conn):
        conn.execute(
            ...
            (
                ...
                "bitset/v1",
                "0" * 64,
                base64.b64encode(b"\x00" * ((int(item["n_vars"]) + 7) // 8)).decode(
                    "ascii"
                ),
                "received",
                "unknown_beta",
...
def test_identity_alignment_under_coldkey_collapse(tmp_path, monkeypatch):
    ...
    def _map(conn):
        conn.execute(
            ...
            "VALUES (?, ?, ?)",
            (kp.ss58_address, coldkey, _now_iso()),
        )
...
def test_launch_profile_requires_pinned_eval_signing_key_for_env_boot(
...
def test_lazy_issuance_mints_at_cnf_fetch_and_bridges(tmp_path, monkeypatch):
    ...
    page = client.get(
        "/v2/synthetic-boolean/per-miner/challenges?limit=2", headers=_read_headers(kp)
    )
    assert page.status_code == 200, page.text
    payload = page.json()
    assert payload["issuance"] == "lazy"
