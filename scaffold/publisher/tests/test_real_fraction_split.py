...
def test_fraction_ten_percent_is_roughly_ten_percent(monkeypatch):
    ...
    n = 400
    real = sum(
        1 for seq in range(n) if pm.generate_instance(HOTKEY, EPOCH, 1, seq)[2] is None
    )
    frac = real / n
    ...
    for raw in (None, "0.0", "0.1", "0.5", "1.0"):
        ...
        for tier in (1, 2):
            for seq in range(12):
                ...
                assert pm.uses_real_instance(HOTKEY, EPOCH, tier, seq) == (
                    planted is None
                ), (
    ...
    for tier in (1, 2):
        for seq in range(12):
            ...
            assert sol is not None, (
                f"real tier{tier} seq{seq} was UNSAT — must be solvable"
            )
