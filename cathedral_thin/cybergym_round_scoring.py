...
def _tiebreak(nonce: bytes, source_epoch: int, hotkey: str) -> str:
    material = nonce + b"\x00" + str(int(source_epoch)).encode() + b"\x00" + hotkey.encode()
...
def compose_round_board(
    ...
    *,
    ...
    if not isinstance(nonce, (bytes, bytearray, str)):
        raise RoundScoringError("nonce must be bytes/bytearray/str (the chain-anchored nonce)")
    nb = bytes(nonce) if isinstance(nonce, (bytes, bytearray)) else nonce.encode("utf-8")
    if not nb:
        raise RoundScoringError("a non-empty nonce is required (payout-decisive tie-break)")
    totals = {hk: _q(Decimal(s)) for hk, s in round_scores.items()}
    ordered = sorted(totals, key=lambda hk: (-totals[hk], _tiebreak(nb, source_epoch, hk), hk))
    winners = [h for h in ordered if totals[h] > 0][:WINNER_SLOTS]
    shares = award_shares(len(winners))
    by_hk = dict(zip(winners, shares))
    standings = tuple(
        RoundStanding(miner_hotkey=h, score=totals[h], rank=i + 1, lane_share=by_hk.get(h, Decimal(0)))
...
__all__ = [
    "RUNNER_UP_SHARES", "WINNER_SLOTS", "BASE", "RoundScoringError",
    "round_score_base100", "award_shares", "RoundStanding", "RoundBoard", "compose_round_board",
]
