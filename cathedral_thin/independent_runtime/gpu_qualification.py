...
class GpuPrelaunchConfig:
    ...
    def load(cls, path: str | Path) -> "GpuPrelaunchConfig":
        ...
        if (
            ...
            and data.get("schema") == "cathedral_gpu_g4_prelaunch_v1"
        ):
            keys = {
                "schema",
                "enabled",
                ...
                "validator_hotkey",
                "units_per_device",
                "trusted_operators_hex",
            }
            if set(data) != keys or data["enabled"] is not True:
                raise IndependentLiveError(
                    "G4 qualification requires its explicit config"
                )
            if (
                data["network"] not in {"finney", "test"}
                or type(data["netuid"]) is not int
                or not 0 <= data["netuid"] <= 65535
            ...
            if (
                ...
                or any(
                    ...
                    or not re.fullmatch(r"[A-Za-z0-9_.:-]{1,96}", k)
                ...
                raise IndependentLiveError(
                    "G4 requires explicit approved operator public keys"
                )
            return cls(
                data["network"],
                data["netuid"],
                data["validator_hotkey"],
                data["units_per_device"],
                "",
                {},
                0,
                "",
        ...
        if (
            ...
            raise IndependentLiveError(
                "GPU prelaunch scoring or chain context is invalid"
            )
        _require_hotkey(data["validator_hotkey"], "validator hotkey")
        ...
        if (
            ...
            or any(
                ...
                or re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.:-]{0,95}", v) is None
                for v in ids
            )
        ...
        if any(
            not isinstance(k, str) or not k or len(v) != 32 for k, v in trusted.items()
        ):
        ...
        return cls(
            ...
            data["registry_path"],
            trusted,
            data["minimum_registry_release"],
            data["registry_state_path"],
            tuple(ids),
        )
...
class ProductionGpuVerifier:
    ...
    def __init__(self, config: GpuPrelaunchConfig):
        if (
            G4_WORKER_PROFILE_ID in config.profile_ids
            or config.trusted_operators is not None
        ):
            raise IndependentLiveError(
                "G4 requires the distinct approved-operator verifier"
            )
        ...
        self.profiles = {
            name: gpu_profile_from_registry(snapshot, name)
    ...
    def verify(self, components, nonce, hotkey, binding, profile_id):
        ...
        if set(by_kind) != {EvidenceKind.TDX, EvidenceKind.GPU_CC}:
            raise IndependentLiveError(
                "GPU composite must contain TDX and GPU evidence"
            )
        result = verify_composite_gpu(
            by_kind[EvidenceKind.TDX],
            by_kind[EvidenceKind.GPU_CC],
            nonce,
            self.cpu_policy,
        ...
        identities = tuple(
            sorted(
                gpu_identity_policy_digest(v) for v in result.gpu_component.identity_set
            )
        )
        return {
            "device_identity_digests": identities,
...
def _admit(origin, miner, transport, verifier, row):
    ...
    if (
        ...
        or type(capability["elements"]) is not int
        or capability["elements"] != ELEMENTS
        or capability["status"] != "registered"
        or capability["verified"] is not False
    ):
    ...
    if capability["device_identity_digests"] != list(expected):
        raise IndependentLiveError(
            "GPU capability device set differs from signed profile"
        )
    row.update(
        profile_id=profile_id,
        gpu_count=len(expected),
        declared_device_identity_digests=expected,
    )
    ...
    expected_schema = (
        "cathedral_gpu_provider_evidence_v1"
        if profile_id == G4_WORKER_PROFILE_ID
    ...
    verified = verifier.verify(
        envelope["evidence"], nonce, miner.hotkey, binding, profile_id
    )
    if verified["device_identity_digests"] != expected:
        raise IndependentLiveError(
            "GPU verified device set differs from signed profile"
        )
    if profile_id == G4_WORKER_PROFILE_ID:
    ...
    return {
        ...
        "gpu_count": len(expected),
        "verified": True,
        "channel_id": binding.hex(),
        "device_identity_digests": expected,
        "machine_id": verified["machine_id"],
        "admission_digest": verified["component_digest"],
        "evidence_digest": verified["component_digest"],
        "verified_at": _utc(),
    }
...
def _work(row, transport, verifier):
    ...
    verified = verifier.verify(
        ...
        row["hotkey"],
        bytes.fromhex(row["channel_id"]),
        row["profile_id"],
    )
    if (
        verified["device_identity_digests"] != row["device_identity_digests"]
        or verified["machine_id"] != row["machine_id"]
        or (
            row["profile_id"] == G4_WORKER_PROFILE_ID
            and (
                verified.get("provider_instance_id") != row.get("provider_instance_id")
                or verified.get("worker_key_digest") != row.get("worker_key_digest")
            )
        )
    ):
        raise IndependentLiveError(
            "GPU completion changed admitted device or CPU identity"
        )
    return _digest(
        {
            "request": request,
...
def reject_duplicate_devices(rows: list[dict]) -> None:
    ...
    identities = Counter(
        device for row in verified for device in row["device_identity_digests"]
    )
    ...
    instances = Counter(
        ...
        for row in verified
        if row.get("provider_instance_id")
    )
    keys = Counter(
        row["worker_key_digest"] for row in verified if row.get("worker_key_digest")
    )
    for row in verified:
        if (
            any(identities[v] > 1 for v in row["device_identity_digests"])
            or channels[row["channel_id"]] > 1
            or endpoints[row["endpoint"]] > 1
            or (
                row.get("provider_instance_id")
                and instances[row["provider_instance_id"]] > 1
            )
...
def qualify_gpu_round(
    miners: Sequence[ServingAxon],
    *,
    config: GpuPrelaunchConfig,
    keypair: Any,
    verifier: ProductionGpuVerifier,
    transport_factory=HttpsEvidenceTransport,
) -> GpuRound:
    if str(getattr(keypair, "ss58_address", "")) != config.validator_hotkey:
        raise IndependentLiveError(
            "GPU access signer differs from configured validator"
        )
    if len(miners) > MAX_MINERS or len({m.uid for m in miners}) != len(miners):
        raise IndependentLiveError(
            "GPU miner snapshot is invalid or exceeds round bound"
        )
    rows, transports = [], {}
    ...
    for miner in miners:
        if time.monotonic() >= deadline:
            raise IndependentLiveError(
                "GPU round deadline exceeded before complete deduplication"
            )
        primary = axon_origin(miner.ip, miner.port)

        def signed():
            return SignedValidatorTransport(
                transport_factory(
                    timeout=150 if config.trusted_operators is not None else 90,
        ...
        try:
            fleet = fetch_worker_fleet(
                primary_origin=primary, worker_hotkey=miner.hotkey, transport=signed()
            )
        ...
        def admit_endpoint(endpoint):
            ...
            try:
                ...
                row["reason"] = (
                    "no_gpu"
                    if exc.status == 404 and row["profile_id"] is None
                    else "endpoint_unavailable"
                )
            except Exception as exc:
                category = getattr(exc, "category", "failed")
                row["reason"] = "admission_" + (
                    category
                    if category
                    in {
                        "unavailable",
    ...
    with ThreadPoolExecutor(
        max_workers=8 if config.trusted_operators is not None else 1
    ) as pool:
...
def enforce_g4_bundles(rows: list[dict]) -> None:
    ...
    for members in grouped.values():
        complete = (
            len(members) == G4_BUNDLE_SIZE
            and all(
                row.get("verified") is True
                and row.get("eligible") is True
                and len(row.get("device_identity_digests", ())) == 1
                for row in members
            )
            and len({row.get("provider_instance_id") for row in members})
            == G4_BUNDLE_SIZE
            ...
            and len({row["device_identity_digests"][0] for row in members})
            == G4_BUNDLE_SIZE
        )
        if not complete:
            for row in members:
...
def direct_gpu_plan(
    ...
    miners: Sequence[ServingAxon],
    *,
    cpu_machine_ids: Mapping[int, tuple[str, ...]] | None = None,
):
    ...
    for row in checked:
        ...
        if (
            hotkeys.get(uid) != row["hotkey"]
            or row.get("verified") is not True
            or row.get("reason") != "verified_work"
            or not row.get("evidence_digest")
            or not ids
            or len(set(ids)) != len(ids)
            or any(v in all_devices for v in ids)
        ):
            raise IndependentLiveError(
                "GPU positive score has invalid identity or work proof"
            )
    ...
    scores = tuple(
        (uid, len(cpu.get(uid, ())) + len(devices[uid]) * config.units_per_device)
        for uid in sorted(hotkeys)
    )
    uids, weights = (
        zero_burn_vector(scores, hotkeys) if any(v for _, v in scores) else ((), ())
    )
    return {
        "schema": PLAN_SCHEMA,
        "prelaunch_only": True,
        ...
        "network": config.network,
        "netuid": config.netuid,
        "discovery_complete": not any(
            row.get("profile_id") is None and row.get("reason") != "no_gpu"
            for row in result.rows
        ),
        "scoring_policy": {
            "cpu_machine_units": 1,
            "gpu_device_units": config.units_per_device,
            "workload_id": "cuda_i32_vector_v1",
        },
        "raw_scores": [list(v) for v in scores],
        "uids": list(uids),
        ...
        else "native_tdx_gpu_composite",
        "private_customer_work": False,
        "evidence_digest": _digest(result.rows),
    }


def provider_directory(
    result: GpuRound, config: GpuPrelaunchConfig, verifier: ProductionGpuVerifier
):
    ...
    for row in result.rows:
        if row.get("profile_id") is not None:
            public_profile = (
                ...
                if row["profile_id"] == G4_WORKER_PROFILE_ID
                else row["profile_id"]
            )
            grouped.setdefault((row["uid"], row["hotkey"], public_profile), []).append(
                row
            )
    providers = []
    for (uid, hotkey, profile_id), rows in sorted(grouped.items()):
        ...
        if profile_id == G4_BUNDLE_PROFILE_ID:
            count = len(
                {
                    v
                    for row in rows
                    for v in row.get("declared_device_identity_digests", ())
                }
            )
            ...
            verified = (
                verified
                and len(rows) == G4_BUNDLE_SIZE
                and count == G4_BUNDLE_SIZE
                and len({row.get("provider_instance_id") for row in rows})
                == G4_BUNDLE_SIZE
                and all(row.get("provider_instance_id") for row in rows)
                and len({row.get("worker_key_digest") for row in rows})
                == G4_BUNDLE_SIZE
        ...
        reason = (
            "verified_work_prelaunch"
            if work
            else (
                "duplicate_gpu_or_channel"
                if any(row["reason"] == "duplicate_gpu_or_channel" for row in rows)
                else "g4_requires_eight_verified_instances"
                if profile_id == G4_BUNDLE_PROFILE_ID and not verified
                else "verified_admission_work_failed"
                if verified
        ...
        material = [
            {
                "admission": row.get("admission_digest"),
                "work": row.get("evidence_digest"),
            }
            for row in rows
        ]
        providers.append(
            {
                ...
                "eligible": False,
                "reason": reason,
                "evidence_digest": _digest(material) if verified else None,
                "verified_at": max(row["verified_at"] for row in rows)
                if verified
                else None,
            }
        )
    return {
        ...
        "discovery": {
            "complete": not failed_miners,
            "failed_miners": len(failed_miners),
        },
        "network": config.network,
        "netuid": config.netuid,
        "admission": {
            "open": True,
            "reason": "Configured profiles accept qualification; live rewards are not enabled.",
        },
        "profiles": [
            {
                "id": G4_BUNDLE_PROFILE_ID
                if profile.profile_id == G4_WORKER_PROFILE_ID
                else profile.profile_id,
                "model": ", ".join(sorted(profile.allowed_models)),
                "gpu_count": G4_BUNDLE_SIZE
                if profile.profile_id == G4_WORKER_PROFILE_ID
                else len(profile.expected_device_identity_digests),
                "cpu_tee": "amd_sev"
                if profile.profile_id == G4_WORKER_PROFILE_ID
                else "intel_tdx",
                "status": "qualification",
                "reason": "CPU host attestation not verified; eight single-GPU VMs; no live rewards."
                if profile.profile_id == G4_WORKER_PROFILE_ID
                else "Prelaunch hardware and work qualification; no chain writes.",
            }
...
class AccessRequestSigner:
    ...
    def __init__(self, executable: str, config: GpuPrelaunchConfig):
        if not Path(executable).is_absolute() or not Path(executable).is_file():
            raise IndependentLiveError(
                "request signer must be an explicit executable path"
            )
    ...
    def sign(self, payload: bytes) -> bytes:
        ...
        if (
            ...
            or request.get("schema") != "cathedral_validator_request_v1"
        ...
        try:
            ...
            raise IndependentLiveError(
                "request signer returned an invalid signature"
            ) from None
        if len(signature) != 64:
...
def read_registered_miners(subtensor, config, expected_genesis_hash):
    ...
    for value in metagraph.validator_permit:
        ...
        if type(value) is not bool:
            raise IndependentLiveError(
                "GPU validator permit is not an explicit boolean"
            )
        permits.append(value)
    if (
        len(uids) != len(hotkeys)
        or len(uids) != len(permits)
        ...
        raise IndependentLiveError(
            "GPU qualification validator identity or permit is invalid"
        )
    validator_uids = {uid for uid, permit in zip(uids, permits) if permit}
    miners = tuple(
        m for m in scan_axons(metagraph).serving if m.uid not in validator_uids
    )
    identities = dict(zip(uids, hotkeys))
    if any(identities.get(m.uid) != m.hotkey for m in miners):
        raise IndependentLiveError(
            "GPU miner identities differ from finalized snapshot"
        )
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="GPU hardware/work qualification, never chain writes"
    )
    ...
    try:
        ...
        try:
            miners, anchor = read_registered_miners(
                subtensor, config, args.expected_genesis_hash
            )
            result = qualify_gpu_round(
                miners, config=config, keypair=signer, verifier=verifier
            )
        finally:
        ...
        document = {
            "schema": "cathedral_gpu_qualification_v1",
            "chain_write": False,
            "verdict": "PASS" if plan["uids"] else "NOT_PROVEN",
            "anchor": anchor,
            "observed_at": result.observed_at,
            "plan": plan,
            ...
            json.dumps(
                {
                    ...
                    "chain_write": False,
                    "eligible_providers": sum(
                        row["eligible"] is True for row in result.rows
                    ),
                }
            )
        )
        ...
        if args.gpu_directory_output:
            atomic_json(
                args.gpu_directory_output,
                {
                    "schema": "cathedral.gpu.providers.unavailable.v1",
